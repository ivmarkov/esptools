...
fn main() -> anyhow::Result<()> {
    ...
    let Some(image) = image else {
        anyhow::bail!("Unsupported target: os={target_os}, arch={target_arch}, env={target_env}");
    };
    ...
    fs::write(&zip_file_path, zip_response.bytes()?)?;

    let zip_file_path = zip_file_path.canonicalize()?;
    ...
    let mut tools = ZippedTools::new(
        fs::File::open(&zip_file_path)?,
        target_os.as_str() == "windows",
    )
    .unwrap();

    for tool in Tool::iter() {
        let sha1 = tools.sha1_of(tool)?;
        println!(
